...
export const PlaceOrder = () => {
  ...
  const { data: userData } = useQuery({
    queryKey: ["user"],
    queryFn: async () => {
      const res = await axios.get(`${import.meta.env.VITE_ORDERING_SERVICE_PREFIX}/users/get-user`, {
  ...
  const { data: cartData, error: cartError, isLoading: cartLoading } = useQuery({
    queryKey: ["cart"],
    queryFn: async () => {
      const res = await axios.get(`${import.meta.env.VITE_ORDERING_SERVICE_PREFIX}/cart`, {
        withCredentials: true,
      });
      return res.data;
    },
  ...
  const { data: proData, error: proError, isLoading: proLoading } = useQuery({
    queryKey: ["product"],
    queryFn: async () => {
      const res = await axios.get(`${import.meta.env.VITE_ORDERING_SERVICE_PREFIX}/cart/product`, {
        withCredentials: true,
      });
      return res.data;
    },
  ...
  const { mutate: createOrder, isPending } = useMutation({
    mutationFn: async ({ cartId, resId, total, status }) => {
      const res = await axios.post(
          `${import.meta.env.VITE_ORDERING_SERVICE_PREFIX}/orders/${cartId}/${resId}`,
          { total, status },
          { withCredentials: true }
      );
      return res.data;
  ...
  const {mutate:refund,isPending:isRefunding} = useMutation({
    mutationFn:async ({paymentIntentId,totalPrice}) =>{
      const response = await axios.post(`${import.meta.env.VITE_ORDERING_SERVICE_PREFIX}/orders/refund`,{
        paymentIntentId,
        totalPrice
      },{
        withCredentials: true

...
//             imageUrl,
//             photo,
//             restaurant: restaurantId //link menu item to restaurant
//         });
...
//     }catch (err) {
//         res.status(500).json({ message: "Error creating menu item", err: err.message });
//     }
// };
...
// const updatedMenuItem = async (req, res) => {
//     try{
//         const menuItemId = req.params.id;  //getting menuItem id from URL
//         const updateData = req.body;
//         const userId = req.user.id;
...
//         if(menuItem.restaurant.owner.toString() !== userId){   //menuItem.restaurant.owner: After populate, we can access the owner's ID.
//             return res.status(403).json({ message: "You can only update itemm from your own restaurant" });
//         }

//         const updatedMenuItem = await MenuItemSchema.findByIdAndUpdate(menuItemId, updateData, { new: true });

//         res.status(200).json({ message: "Menu Item updated successfully", updatedMenuItem })
//     } catch (err) {
//         res.status(500).json({ message: "Error updating menu item", err: err.message });
//     }
// };
